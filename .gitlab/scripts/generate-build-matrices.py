...
def write_child_pipeline(chunks: list[list[str]], out_path: str = "build-pipeline.yml") -> None:
    ...
    # Optional concurrency cap: if set, chunks are distributed across N
    # resource_group slots so at most N run at the same time. Leave unset
    # (or 0) to let every chunk run in parallel.
    try:
        build_concurrency = int(os.getenv("BUILD_CONCURRENCY", "0"))
    except ValueError:
        build_concurrency = 0

    # Max heap for the Gradle JVM within each chunk (e.g. "4g", "2048m").
    # Kotlin compiler workers also inherit this via kotlin.daemon.jvm.options.
    max_mem = os.getenv("BUILD_MAX_MEMORY", "4g").strip() or "4g"
    ...
    lines += [
        ...
        # Cap heap for the Gradle JVM; -XX:MaxRAMPercentage is a safety net
        # for any child JVMs that honor it. Daemon disabled — no state reuse
        # benefit in ephemeral CI containers, and the daemon process itself
        # is a non-trivial amount of retained memory.
        f'    GRADLE_OPTS: "-Dorg.gradle.daemon=false -Xmx{max_mem} -XX:MaxRAMPercentage=60"',
        # Kotlin compile daemon: single worker, same heap cap. Without this,
        # Kotlin spawns per-module workers that each allocate their own heap.
        f'    KOTLIN_DAEMON_JVM_OPTIONS: "-Xmx{max_mem}"',
        "  retry:",
        "    max: 2",
        "    when:",
        ...
        "  script:",
        # --no-parallel: don't run multiple project builds at once.
        # --max-workers=1: limit Gradle's worker pool to a single worker.
        # -Dkotlin.compiler.execution.strategy=in-process: run kotlinc in
        #   the Gradle JVM instead of spawning a separate daemon (one heap,
        #   not two).
        '    - ./gradlew --no-parallel --max-workers=1 -Dkotlin.compiler.execution.strategy=in-process $MODULES',
        "  after_script:",
        "    - rm -f signingkey.jks",
        "  artifacts:",
        "    when: on_success",
    ...
    if not chunks:
        ...
        for i, chunk in enumerate(chunks, start=1):
            ...
            lines.append(f"build-chunk-{i}:")
            lines.append("  extends: .build-template")
            if build_concurrency > 0:
                slot = ((i - 1) % build_concurrency) + 1
                lines.append(f"  resource_group: build-slot-{slot}")
            lines += [
                "  variables:",
